  ...
  <div class="classroom-manager">
    <div class="login-button-container">
      ...
      >
        <path
          fill-rule="evenodd"
          d="M6 3.5a.5.5 0 0 1 .5-.5h8a.5.5 0 0 1 .5.5v9a.5.5 0 0 1-.5.5h-8a.5.5 0 0 1-.5-.5v-2a.5.5 0 0 0-1 0v2A1.5 1.5 0 0 0 6.5 14h8a1.5.5 0 0 0 1.5-1.5v-9A1.5 1.5 0 0 0 14.5 2h-8A1.5 1.5 0 0 0 5 3.5v2a.5.5 0 0 0 1 0z"
        />
        <path
          fill-rule="evenodd"
    ...
    <div class="tabs-container">
      <button
        ...
        :class="{
          'active-tab': tab.id === activeTabId,
          'temporary-tab': tab.id === 'temp-randomized',
        }"
    ...
    <div class="tabs-container action-buttons">
      ...
      <button @click="saveAsNewTab" v-if="currentTab && isLoggedIn">
        選択内容を保存
      </button>
    </div>
    ...
    />
    <div v-else class="loading-message">教室のレイアウトを読み込み中．．．</div>
    <div class="tabs-container">
      <button @click="downloadCurrentTab" v-if="currentTab">
        Excelにダウンロード
      </button>
    </div>
  </div>
  ...
  methods: {
    ...
    async fetchAndProcessClassrooms(querySnapshot) {
      ...
      querySnapshot.forEach((doc) => {
        const data = doc.data();
        const assignments = Array.isArray(data.studentAssignments)
      ...
      this.tabs = loadedTabsData.map((tab) => {
        const rehydratedStudents = tab.studentAssignments
          .map((assignment) => {
            const fullStudentData = this.allStudentsMap.get(
              assignment.studentId,
            );
            return fullStudentData
              ? { ...fullStudentData, deskNumber: assignment.deskNumber }
              : { ...this.emptyStudentPlaceholder };
          })
          .filter((s) => !s.isEmpty);
        const { deskLayout, studentsWithDeskNumbers } =
          this.assignStudentsToDesks(rehydratedStudents);
        return {
          ...tab,
          studentAssignments: studentsWithDeskNumbers.map((s) => ({
            studentId: s.id,
    ...
    async initializePage() {
      ...
      try {
        ...
        console.error("Failed to load initial classroom data:", error);
        this.initialLoadComplete = true;
      }
    },
    ...
    startRealtimeListeners() {
      if (!auth.currentUser || !this.isFirestoreReady) {
        console.warn(
          "Cannot set up real-time listeners: user not authenticated or Firestore not ready.",
        );
        return;
      }
      ...
      this.tabsUnsubscribe = watchClassrooms(
        appId,
        async (querySnapshot) => {
    ...
    async saveMasterStudentListInit() {
      if (!auth.currentUser || !this.isFirestoreReady) return;

      if (this.masterListSaved) return;

      const students = await getStudents(this.schoolId);

      if (students.length > 0) return;

      await saveStudents(this.schoolId, this.masterStudentList);

      const classrooms = await getClassrooms(appId);

      if (classrooms.length == 0) {
        console.log(
          "No classroom tabs found. Creating initial layouts from cLists.",
        );
    ...
    assignStudentsToDesks(studentList) {
      ...
      const unassignedStudents = studentsToAssign.filter(
      ...
      desks.forEach((desk) => {
        while (desk.students.length < 2) {
          if (unassignedIndex < unassignedStudents.length) {
            ...
            desk.students.push({
              ...this.emptyStudentPlaceholder,
              id: `empty-${desk.name}-${desk.students.length + 1}`,
            });
          }
        }
      });
    ...
    async addTab(title, studentsArray, autoSave = false) {
      const newTabId =
        this.tabs.length > 0 ? Math.max(...this.tabs.map((t) => t.id)) + 1 : 1;
      ...
      const { deskLayout, studentsWithDeskNumbers } =
        this.assignStudentsToDesks(studentsArray);
    ...
    async randomizeCurrentList() {
      ...
      const studentsToRandomize = this.masterStudentList.filter(
      ...
      for (const deskNumber of availableDesks) {
        ...
        while (deskStudents.length < 2 && studentsToRandomize.length > 0) {
          ...
          if (studentToPlace) {
            ...
            studentsToRandomize.splice(
              studentsToRandomize.findIndex((s) => s.id === studentToPlace.id),
              1,
            );
          } else {
            break;
      ...
      const existingTempIndex = updatedTabs.findIndex(
        (t) => t.id === tempTabId,
      );
    ...
    async saveAsNewTab() {
      ...
      const studentsToSave = this.currentTab
        ? this.currentTab.deskLayout.flatMap((desk) =>
            desk.students.filter((s) => !s.isEmpty),
          )
        : [...this.masterStudentList];
    ...
    async saveTabToFirestore(tabData, docId = null) {
      if (!auth.currentUser || !this.isFirestoreReady) {
        console.warn(
          "Firestore not ready or user not authenticated, cannot save.",
        );
        return null;
      }

      try {
        const savedDocumentId = await saveClassroomLayout(
          appId,
          tabData,
          docId,
        );
        ...
        console.log(
          "Classroom layout successfully written with ID:",
          savedDocumentId,
        );
        ...
        console.error(
          "Error writing classroom layout to Firestore:",
          error,
        );
    ...
    getHistoricalData(tabsArray = this.tabs) {
      ...
      tabsArray.forEach((tab) => {
        tab.deskLayout.forEach((desk) => {
          const studentIds = desk.students
            .map((s) => s.id)
            .filter((id) => id !== "empty-slot");
    ...
    downloadCurrentTab() {
      ...
      const getStudent = (deskNumber, studentIndex) => {
        ...
        return desk && desk.students.length > studentIndex
      ...
      const masterData = Array(totalDeskRows + 2)
        .fill()
        .map(() => Array(7).fill(""));
      desks.forEach((desk) => {
      ...
      masterData[totalDeskRows + 1][1] =
        "K09 " + this.dateShortFormat(this.currentTab.creationDate);
      ...
      const codeCellAddress = XLSX.utils.encode_cell({
        r: totalDeskRows + 1,
        c: 1,
      });
      ...
      const instructorCellAddress = XLSX.utils.encode_cell({
        r: totalDeskRows + 1,
        c: 5,
      });
      if (worksheet[instructorCellAddress]) {
        if (!worksheet[instructorCellAddress].s)
          worksheet[instructorCellAddress].s = {};
